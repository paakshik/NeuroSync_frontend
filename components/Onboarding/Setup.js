...
import { Zap, Brain, Clock, Mic, ChevronRight, Sparkles } from 'lucide-react';

export default function ProfileSetup({ onComplete }) {
  const [step, setStep] = useState(1);
  const [profile, setProfile] = useState({ name: "", diagnosis: "General", energy: "medium", focus_time: 15 });
  ...
  const startRecording = async () => {
    ...
    mediaRecorder.current.onstop = async () => {
      ...
      if (data.transcription) setProfile(prev => ({ ...prev, name: data.transcription }));
      setStatus("READY");
      setStep(2);
    };
  ...
  return (
    <div className="fixed inset-0 bg-[#0F172A] z-[100] p-6 font-lexend flex flex-col items-center justify-center overflow-hidden">
      {/* Background Ambient Glows for Sensory Engagement */}
            ...
            <div className="flex flex-col items-center gap-10">
              ...
              <div className="w-full space-y-4">
                ...
                {profile.name.length > 0 && (
                  <button onClick={() => setStep(2)} className="w-full py-5 bg-blue-600 text-white rounded-2xl font-black text-xl shadow-xl hover:bg-blue-500 flex items-center justify-center gap-2 group">
        ...
        {step === 2 && (
          <div className="space-y-4 animate-in slide-in-from-bottom-10 duration-500">
            ...
            {['ADHD', 'Autism', 'Anxiety', 'General'].map(d => (
              <button key={d} onClick={() => { setProfile({...profile, diagnosis: d}); setStep(3); }} className="w-full p-8 bg-slate-800/50 backdrop-blur-xl rounded-[2rem] border-2 border-slate-700 text-2xl font-black text-left flex justify-between items-center text-white hover:border-blue-400 hover:bg-slate-800 transition-all shadow-xl">
          ...
          <div className="space-y-8 animate-in zoom-in-95 duration-500">
            ...
            <div className="bg-slate-800/80 backdrop-blur-2xl p-8 rounded-[2.5rem] border-2 border-slate-700 shadow-2xl space-y-10">
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-3 gap-3">
                  {['low', 'medium', 'high'].map(e => (
                    <button key={e} onClick={() => setProfile({...profile, energy: e})} className={`py-4 rounded-2xl font-black uppercase text-xs transition-all border-2 ${profile.energy === e ? 'bg-blue-600 border-white text-white shadow-[0_0_20px_rgba(59,130,246,0.5)]' : 'bg-slate-900 border-slate-700 text-slate-500'}`}>{e}</button>
                  ))}
                </div>
              </div>
              <div className="space-y-4 text-white">
                ...
                <input type="range" min="5" max="60" step="5" value={profile.focus_time} onChange={(e) => setProfile({...profile, focus_time: parseInt(e.target.value)})} className="w-full h-3 bg-slate-900 rounded-lg appearance-none cursor-pointer accent-blue-500 border border-slate-700" />
              </div>
            </div>
            <button onClick={handleFinalSave} className="w-full py-6 bg-white text-slate-900 rounded-[2rem] font-black text-2xl shadow-[0_20px_40px_rgba(59,130,246,0.3)] hover:scale-[1.02] active:scale-95 transition-all">
              INITIALIZE AI
            </button>
          </div>
        )}
